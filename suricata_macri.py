...
searchQuery = 'mauricio macri'

maxTweets = 10000
#maxTweets = 10000000 # Some arbitrary large number

tweetsPerQry = 30  # this is the max the API permits
fName = 'tweetsmacri.txt' # We'll store the tweets in a text file.
...
conn = pyodbc.connect('DRIVER={SQL Server};'
                      ...
                      'PWD=Quebueno2')
cursor = conn.cursor()
cursor.execute("SELECT isnull (Max(Id),0) FROM sur_listener where id_listener='macri'")
...
with open('tweetsmacri.txt', 'r') as f:
    for line in f:
        ...
        cursor.execute("INSERT INTO sur_listener VALUES ('macri'," + str(tweet['id']) + ",'" + str(tweet['created_at']) + "',N'" + full_text + "','" + str(tweet['favorite_count']) + "','" + str(tweet['retweet_count']) + "','" + str(tweet['in_reply_to_user_id']) + "','" + user_location + "','" + user_screen_name +"','" + str(tweet["user"]["friends_count"]) +"','" + str(tweet["user"]["followers_count"]) + "','" + str(tweet["lang"]) + "','" + country + "','" + latitude + "','" + longitude + "','" + str(scores) + "','" + str(Happy) + "','" + str(Angry) + "','" + str(Excited) + "','" + str(Sad) + "','" + str(Fear) + "','" + str(Bored) + "','" + str(Negative) + "','" + str(Neutral) + "','" + str(Positive) + "','" + str(Education) + "','" + str(Health) + "','" + str(Economy) + "','" + str(Security) + "','" + str(Corruption) + "','" + str(Work) + "','" + str(Transport) + "','" + str(LivingPlace) + "','','')")
        
        conn.commit()
        
#This        
cursor.execute("insert into sur_listener_wc Select Id, Created_At, substring (dbo.fn_CleanTextV2(dbo.fn_CleanAndTrim(dataitem,'','',1)),1,50), 'macri' from (select * from sur_listener where id_listener='macri' and id>(select isnull(max(id),0) from sur_listener_wc where id_listener='macri')) x cross apply dbo.splitString(x.Full_Text,' ') Y option (maxrecursion 0) ")
conn.commit()

cursor.execute("delete from sur_listener_wc where id_listener='macri' and word in (select word from sur_listener_wc_filters where id_listener='macri')")
conn.commit()

cursor.execute("delete from sur_listener_wc where id_listener='macri' and word like '@%'")
conn.commit()
